...
## 2026-05-16 - FATIA 4A MOTOBOY TRANSICOES POS-ACEITE

**Fase:** fundacao/auth-operacao
**O que aconteceu:** Implementado localmente `PATCH /api/deliveries/:id/status` para o motoboy avancar a corrida atribuida em `aceita -> coletada -> em_transito -> entregue`. A rota usa `authenticate`, `requireActiveUser`, `requireRoles('motoboy')`, body strict `{ status }`, resolve `couriers.id` pela sessao e exige `is_online=true`. O service aplica update condicional por `id`, `courier_id` derivado e status anterior esperado, seta somente o timestamp da etapa, trata retry do mesmo status como idempotente e retorna shape sanitizado sem `store_id`/`courier_id`. `GET /api/deliveries/active` agora retorna `aceita|coletada|em_transito` e exclui `entregue`.
**Arquivos modificados (backend):** `src/routes/delivery.routes.ts`, `src/controllers/delivery.controller.ts`, `src/services/delivery.service.ts`, `src/validators/delivery.validators.ts`, `tests/delivery-routes.spec.ts`, `CONTRACTS.md`, `STATUS.md`, `DECISIONS.md`, `LOG.md`
**Agentes utilizados:** Camisa10, ImpactValidator, SecurityValidator, PerformanceValidator, TestEngineer
**Status:** fechado localmente; deploy/smoke de producao pendentes

**Gates pre-codigo:** ImpactValidator/Cetico aprovou com ressalvas de shape sanitizado, active sem hardcode e update condicional; SecurityValidator aprovou com ressalvas de body strict, filtro por courier da sessao, PII e logs; PerformanceValidator aprovou com ressalvas de sem cache/polling/novo indice; TestEngineer aprovou a matriz minima.
...
**Fase:** fundacao/auth-operacao
**O que aconteceu:** Backend publicado em producao nos commits `18c87cd` e `27987f0b54b6747c6dac5a9f5134f4f2c80d8b3e`; frontend publicado nos commits `0a2f2fb` e `20ab39710367bfcb9565246daef292f275e3c370`. GitHub/Vercel reportou `success` e `Deployment has completed` nos dois repos. O smoke autenticado usou usuarios, perfis e entregas ficticios temporarios, com cleanup em `finally`, sem imprimir token, cookie, header Authorization, service role ou secret.
**Arquivos criados:** nenhum
**Arquivos modificados:** `STATUS.md`, `LOG.md`
**Status:** fechado em producao

**Validacoes pos-deploy:** Smoke publico confirmou `GET https://entreggoback.vercel.app/api/health` -> `200`, `GET /api/deliveries/<uuid>` sem token -> `401 AUTH_REQUIRED`, `https://entreggo.vercel.app/loja/entregas/<uuid>` -> `200` e chunk publicado da rota com marcadores de detalhe (`Atualizar` e `DELIVERY_NOT_FOUND`). Smoke autenticado contra `https://entreggoback.vercel.app` confirmou loja ativa criando entrega ficticia, motoboy ativo/online aceitando e avancando `aceita -> coletada -> em_transito`, loja dona abrindo `GET /api/deliveries/:id` com `status=em_transito`, `accepted_at`, `collected_at` e `in_transit_at` preenchidos, `delivered_at=null`, outra loja ativa recebendo `DELIVERY_NOT_FOUND`, query `store_id` proibida com `VALIDATION_ERROR`, UUID invalido com `VALIDATION_ERROR`, ausencia de `store_id`, `courier_id`, PII de motoboy, Storage, tokens e headers no detalhe, regressao M-05/RLS ainda aprovada e cleanup completo.

**Fora do escopo preservado:** SQL/migration/RLS/grants/policies, realtime, push/Web Push/VAPID, polling automatico, cancelamento, cron/expiracao automatica, historico admin, pagamento externo, documentos/Storage, GPS/mapa/raio e dados pessoais do motoboy.

## 2026-05-17 - HARDENING POS-M-06 IDENTIFICADORES INTERNOS

**Fase:** fundacao/auth-operacao
**O que aconteceu:** Implementado hardening incremental dos contratos antigos de entrega sem alterar a M-06. `POST /api/deliveries` passou a selecionar e devolver o mesmo shape sanitizado usado pela loja em lista/detalhe, sem `store_id` nem `courier_id`. `POST /api/deliveries/:id/accept` deixou de retornar `courier_id`, mantendo `id`, `status`, `accepted_at`, `created_at`, `expires_at` e `store(name,address)`. Logs operacionais de aceite e status agora registram apenas `event`, `delivery_id`, `result` e, no status, `from_status`/`to_status`; nao registram `courier_id`, payload, PII, token, header, email, endereco ou observacao.
**Arquivos modificados (backend):** `src/services/delivery.service.ts`, `tests/delivery-routes.spec.ts`, `CONTRACTS.md`, `STATUS.md`, `LOG.md`
**Arquivos frontend relacionados:** `src/types/delivery.ts`, `src/lib/__tests__/api.deliveries.test.ts`, `src/components/loja/__tests__/NovaEntregaFlow.test.tsx`, `src/components/motoboy/__tests__/FilaDisponivel.test.tsx`, `src/components/motoboy/__tests__/MotoboyRealFlow.test.tsx`, `CONTRACTS.md`, `STATUS.md`, `LOG.md`
...
**Fase:** fundacao/auth-operacao
**O que aconteceu:** Commits da M-06.1 enviados para `origin/main`: backend `3c168b5cf041bbde7a771c69a9b3c43ec5c005c3` e frontend `5005d847ec57d7835ed9ecf07ca870573dad31f9`. `git ls-remote` confirmou os dois SHAs em `refs/heads/main`.
**Status:** publicado em `origin/main`; smoke publico aprovado com ressalvas de observabilidade

**Validacoes pos-push:** Smoke publico confirmou `GET https://entreggoback.vercel.app/api/health` -> `200`, `GET /api/deliveries/active` sem token -> `401`, `GET /api/deliveries/available` sem token -> `401`, `POST /api/deliveries/<uuid>/accept` sem token -> `401` e `https://entreggo.vercel.app/motoboy` -> `200`. O `gh` local retornou `401 Unauthorized`, entao checks GitHub/Vercel nao foram consultados via CLI. Smoke autenticado nao foi executado porque nao havia credencial segura disponivel no contexto.

**Fora do escopo preservado:** SQL/migration/RLS/grants/policies, realtime, push/Web Push/VAPID, polling automatico, cron, cancelamento, GPS/mapa/raio, Storage/documentos, historico admin, pagamento externo, dados pessoais do motoboy e logs com dados sensiveis.

## 2026-05-17 - M-06.1 FECHAMENTO OPERACIONAL POS-DEPLOY

**Fase:** fundacao/auth-operacao
**O que aconteceu:** Fechamento operacional executado apos confirmacao de credenciais locais seguras em `.env.local` ignorado. Backend local e remoto em `origin/main` confirmados no SHA `5ea06bc6ab421af3b5efec93437ed18d5ae25a35`; frontend local/remoto confirmado no SHA `038eb2bfadb7dc7678bee2947080ae5f1a6428cb`. O smoke publico foi repetido e o smoke autenticado minimo API+UI foi executado em producao com loja, motoboy e entrega ficticios, com cleanup em `finally`.
**Arquivos modificados:** `STATUS.md`, `LOG.md`
**Status:** fechado em producao

**Validacoes:** Smoke publico confirmou `/api/health` `200`, `/api/deliveries/active` sem token `401`, `/api/deliveries/available` sem token `401`, `POST /api/deliveries/<uuid>/accept` sem token `401` e `/motoboy` `200`. Smoke autenticado criou loja e motoboy ficticios, criou entrega pela API de loja, confirmou que a fila do motoboy expunha `store.name`/`store.address` sem `destination_address`/`notes`, aceitou a entrega, confirmou que `/api/deliveries/active` retornava `store.name`, `store.address`, `destination_address` e `notes` para o courier atribuido, e validou via Playwright a UI real `/motoboy` com `Loja solicitante`, `Coleta`, endereco da loja, destino e observacao pos-aceite. A UI/API nao expuseram `store_id`, `courier_id`, `owner_name`, `logo_url`, documentos, `Authorization` ou `Bearer`. Cleanup completo confirmado com `domain_residue=0` e `auth_residue=0`.
...
**Fase:** fundacao/auth-operacao
**O que aconteceu:** A M-07 foi fechada operacionalmente em producao. O SQL remoto informado pelo operador foi verificado com a query segura em `pg_indexes`, retornando o indice `idx_delivery_requests_created_at_id_desc` em `public.delivery_requests (created_at desc, id desc)`. O backend foi publicado em `origin/main` no commit `a258888be13b69515ca4521c931452fac5796df2` e o frontend relacionado no commit `48994109cfd1c559ac4df6b3eddf03de66d46c9b`.
**Arquivos modificados nesta rodada documental:** `STATUS.md`, `LOG.md`
**Agentes utilizados:** Camisa10, DeployObservability, Documentador
**Status:** fechado em producao

**Validacoes locais antes do push:** backend `npm run typecheck`, `npm test` (158), `npm run lint`, `npm run build` e `git diff --check`; frontend `npm run typecheck`, `npm test` (75), `npm run lint`, `npm run build` e `git diff --check`. `git diff --check` sem erro de whitespace, apenas avisos LF/CRLF do Windows.

**Deploy e smoke publico:** GitHub/Vercel retornou `success` para backend e frontend. `GET https://entreggoback.vercel.app/api/health` retornou `200`; `GET https://entreggoback.vercel.app/api/admin/deliveries` sem token retornou `401 AUTH_REQUIRED`; `https://entreggo.vercel.app/admin/entregas` retornou `200`.
